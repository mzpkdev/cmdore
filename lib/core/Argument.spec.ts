...
describe("Argument.parse", () => {
    describe("when value is undefined", () => {
        it("should throw when argument is required", async () => {
            ...
            await expect(Argument.parse(argument, undefined))
        ...
        it("should return the defaultValue result", async () => {
            const argument = { name: "target", defaultValue: () => "production" }
            const result = await Argument.parse(argument, undefined)
    ...
    describe("validation", () => {
        it("should throw when validate returns false", async () => {
            ...
            await expect(Argument.parse(argument, "invalid"))
                .rejects.toThrowError(`An argument "target" does not accept "invalid" as a value.`)
        })
        ...
        it("should run validate before parse", async () => {
            ...
            const argument = {
                name: "target",
                validate: () => { calls.push("validate"); return true },
                parse: (_value: string) => { calls.push("parse"); return "parsed" }
            }
...
describe("Argument.parseVariadic", () => {
    it("should throw when values are empty and argument is required", async () => {
        ...
        await expect(Argument.parseVariadic(argument, []))
            .rejects.toThrowError(`An argument "files" is required.`)
    })
    ...
    it("should return defaultValue when values are empty", async () => {
        const argument = { name: "files", variadic: true, defaultValue: () => ["README.md"] }
        const result = await Argument.parseVariadic(argument, [])
    ...
    it("should return raw values array when no parse function exists", async () => {
        const argument = { name: "files", variadic: true }
        const result = await Argument.parseVariadic(argument, ["a.ts", "b.ts", "c.ts"])
    ...
    it("should throw when validate returns false", async () => {
        const argument = {
            name: "files",
            variadic: true,
            validate: (...values: string[]) => values.every(v => v.endsWith(".ts"))
        }
        await expect(Argument.parseVariadic(argument, ["a.ts", "b.js"]))
            .rejects.toThrowError(`An argument "files" does not accept "a.ts b.js" as a value.`)
    })
})
...
describe("defineArgument", () => {
    it("should return the same argument object", () => {
        const argument = { name: "target" as const, description: "Deploy target" }
