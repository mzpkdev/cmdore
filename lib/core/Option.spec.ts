...
describe("Option.parse", () => {
    describe("when values are undefined", () => {
        it("should throw when option is required", async () => {
            ...
            await expect(Option.parse(option, undefined))
                .rejects.toThrowError(`An option "host" is required.`)
        })
    ...
    describe("validation", () => {
        it("should throw when validate returns false", async () => {
            ...
            await expect(Option.parse(option, ["0"]))
                .rejects.toThrowError(`An option "port" does not accept "0" as an argument.`)
        })
        ...
        it("should run validate before parse", async () => {
            ...
            const option = {
                ...
                parse: (..._values: string[]) => { calls.push("parse"); return "parsed" }
            }
